    ...
    public class LoginController : Controller
    {
        ...
        public ActionResult Index(FormCollection collection)
        {
            try
            {
                ...
                var notMember = db.People.Where(model => model.Groups.Count == 0).SingleOrDefault(model => model.Username == tk && model.Password == mk);
                var check = db.People.Where(model => model.Groups.Count != 0).SingleOrDefault(model => model.Username == tk && model.Password == mk);
