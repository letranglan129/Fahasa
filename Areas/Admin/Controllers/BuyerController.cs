...
namespace Fahasa.Areas.Admin.Controllers
{
    ...
    public class BuyerController : Controller
    {
        ...
        public ActionResult GetCustomers()
        {
            try
            {
                return Json(db.People.Where(x => x.GroupId == null).Select(x => new { x.Id, x.Username, x.Name, x.Gender, x.Street, x.Ward, x.District, x.Province, Birth=x.Birth.ToString(), x.Phone, x.Email, x.Avatar, x.Lock, Groups = x.Groups.Select(g => new { g.Id, g.Name }) }).ToList(), JsonRequestBehavior.AllowGet);
            }
            catch (Exception)
            {
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
